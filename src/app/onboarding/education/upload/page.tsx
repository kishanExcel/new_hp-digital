...
const Checklist = () => {
  ...
  const handleSkip = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // router
    //   .push("/onboarding/education/industry")
    //   .catch((error) => console.error("Navigation error:", error));

    try {
      await router.push("/onboarding/education/industry");
  ...
  const handleContinue = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // if (selectedFile) {
    //   updateContextData({ image: selectedFile.name });
    // }

    if (context) {
      if (selectedFile) {
        context.updateContextData({ image: selectedFile.name });
      }
      context.updateContextData({ jobTitle });
    }
  ...
  return (
    <Layout
      hHeading=""
      Childrens={
        <div className="px-[43px] flex items-center w-full flex-col relative ">
          <ProgressBar count={7} />

          <div className="text-center">
            <h1 className="text-[22px] md:text-3xl lg:text-[45px] text-darkSilverColor font-bold mt-[27px] lg:mt-[35px]">
              Upload a photo and select your title.
            </h1>
            <p className="text-[14px] md:text-lg lg:text-[30px] leading-normal pt-2 lg:py-6 tracking-tight text-darkSilverColor">
              Upload a profile picture. Then, add your job title.
            </p>
            <div>
              <label
                className="text-[14px] md:text-lg pt-2 lg:pt-6 lg:text-[22px] text-darkSilverColor"
          ...
          <div
            className="flex justify-center text-[14px] md:text-lg lg:text-[30px] text-darkSilverColor font-bold"
          ...
          <div className="absolute lg:hidden transform translate-x-0 pt-4 bottom-[-90px] ">
