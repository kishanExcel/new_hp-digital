...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method !== 'GET') {
        ...
        try {
            ...
            const url = `https://api.sandbox.fortis.tech/v1/transactions/${id}`;
            ...
            if (!fortisResponse.ok) {
                ...
                res.status(fortisResponse.status).json({ error: 'Failed to retrieve transactions', details: errorDetails });
            } else {
