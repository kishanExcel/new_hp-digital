...
//             const apiUrl = `https://staging-api.ldex.co/listings-management/api/v2/businesses/${businessId}/listings`;
...
//                 res.status(jsonResponse.status).json({ error: 'Failed to retrieve business listings', details: errorDetails });
//             }
//         } catch (error) {
//             // Handle unexpected errors
//             if (error instanceof Error) {
//                 console.error('Unexpected error during fetching business listings:', error.message);
//                 res.status(500).json({
//                     error: 'Internal Server Error',
//                     message: error.message
//                 });
//             } else {
//                 console.error('Unexpected error during fetching business listings:', error);
//                 res.status(500).json({
//                     error: 'Internal Server Error',
    ...
    try {
        ...
        if (jsonResponse.ok) {
            ...
            return NextResponse.json(
                { error: 'Failed to retrieve business listings', details: errorDetails },
